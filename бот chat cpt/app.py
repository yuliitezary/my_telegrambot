...
bot = telebot.TeleBot("6028225386:AAGnM2iWr76I9fSkyRV4pRFJ1Gbi_bSlKXk") #тут токен
...
def photo(message):
    if message.caption and message.caption == "/edit":
        ...
        if ban:
            ...
            if result == 'no key':
                bot.reply_to(message, f"Закончились активные ключи OpenAi\nПерейдите в бота и через комануд /token <api key> добавьте новый")
            elif result == 'error':
    ...
    if ban:
        ...
        try:
            if token != '' or token != ' ':
                ...
                openai.Completion.create(engine="text-davinci-002", prompt="moldova")
